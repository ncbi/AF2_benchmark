...
def score_decoy(target_seq, decoy_prot, model_runner, name):
  ...
  if decoy_seq_in == target_seq:
    print("DEBUG")
    print("decoy_prot.residue_index-1")
    print(decoy_prot.residue_index - 1)
    print("")
    print("np.arange(len(target_seq))")
    print(np.arange(len(target_seq)))
    ...
    print('target_seq')
    print(target_seq)
    print("decoy_prot.residue_index")
    print(decoy_prot.residue_index)
    print("decoy_seq_in")
    print(decoy_seq_in)
  ...
  if args.mask_sidechains_add_cb:
    ...
    print("DEBUG: unhashable type: 'numpy.ndarray' error message")
    print("backbone_modelled = jnp.all(decoy_prot.atom_mask[:,[0,1,2]] == 1, axis=1)")
    print(backbone_modelled)
    print("decoy_prot.residue_index[backbone_modelled] - 1")
    print(decoy_prot.residue_index[backbone_modelled] - 1)
    
    backbone_idx_set = set(np.squeeze(decoy_prot.residue_index[backbone_modelled]) - 1)
...
def compute_tmscore(pdb_pred, pdb_native, test_len=True):
  ...
  print("DEBUG: tmscore_output")
  dg_lines = tmscore_output.split('\n')
  ...
  try:
    ...
    print("DEBUG: re outputs")
    print(tm_re.search(tmscore_output))
    print(ref_len_re.search(tmscore_output))
    print(common_re.search(tmscore_output))
  ...
  if test_len:
    ...
    assert reflen == common, cmd  # asserts if length of residues equals number of residues in common
...
''' Checks if the results from the finished decoys are already there
'''
finished_decoys = []
...
for n in natives_list:
  ...
  print("DEBUG: seq native")
  print(seq_native)
  print("\nDEBUG: length of seq native")
  print(len(seq_native))
...
"""
natives_list: list of native pdbs. pdb names are directly from user input
seq_native: the native sequence
"""